        ...
        SubShader
    {
        Pass
        {
            ...
            SHADERDATA VSMain(float4 vertex:POSITION, float2 uv : TEXCOORD0)
            {
                ...
                vs.uv = TRANSFORM_TEX(uv, _TextureMap);
            ...
            {
                fixed4 col = tex2D(_TextureMap, ps.uv);
                col = _Color * (1 - col.a) + col * col.a;
                return lerp(float4(_ShadowBrightness,_ShadowBrightness,_ShadowBrightness,1) * col, col, SHADOW_ATTENUATION(ps));
            }
