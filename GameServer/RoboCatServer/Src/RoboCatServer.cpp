...
void RoboCatServer::Update()
{
	...
	if (state != 0)
	{
		NetworkManagerServer::sInstance->SetStateDirty(GetNetworkId(), state);
	}
...
void RoboCatServer::HandleShooting()
{
	...
	if( mIsShooting && Timing::sInstance.GetFrameStartTime() > mTimeOfNextShot )
	{
		...
		//fire!
		YarnPtr yarn = std::static_pointer_cast< Yarn >( GameObjectRegistry::sInstance->CreateGameObject( 'BULT' ) );
		yarn->InitFromShooter( this );

		// Only shoot by click instead lots of firepower
		mIsShooting = false;
	}
}

void RoboCatServer::HandleSwitchingColor() 
{
	if (mIsSwitchColor)
	{
